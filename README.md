...
- Python 3 (**3.10+** recommended; **3.12** is supported with the pinned stack below)
...
5. Install the project in editable mode (dependencies and dev tools come from `pyproject.toml`):

   ```bash
   pip install -e ".[dev]"
   ```

   Alternatively, `requirements.txt` is a one-line alias for the same editable install:
...
Run the ETL from the **`ecom-etl-data`** folder using the **venv** where you installed the project:
...
Paths and the SQLite table name are read from optional **`ECOM_ETL_*`** variables (see [`src/etl_config.py`](src/etl_config.py)). Examples:
...
Example using the committed sample file:
...
On **Python 3.12**, Spark’s `toPandas()` path can fail (`distutils` removed). This project loads staging **Parquet** with **pandas + pyarrow** instead ([`src/jobs/load.py`](src/jobs/load.py)). The dependency set includes **`setuptools`** so PySpark 3.3’s remaining `distutils` imports still resolve.
...
Configuration lives in [`pyproject.toml`](pyproject.toml). **AI / automation** notes are in [`AGENTS.md`](AGENTS.md).
